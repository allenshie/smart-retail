...
from third_party.yolov8.ultralytics.utils import (ASSETS, DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, LOGGER, RANK, ROOT, RUNS_DIR,
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    if args[0] == 'login':
        ...
        # Log out from third_party.yolov8.ultralytics HUB
        hub.logout()
...
def entrypoint(debug=''):
    ...
    if 'rtdetr' in model.lower():  # guess architecture
        from third_party.yolov8.ultralytics import RTDETR
        model = RTDETR(model)  # no task argument
    elif 'fastsam' in model.lower():
        from third_party.yolov8.ultralytics import FastSAM
        model = FastSAM(model)
