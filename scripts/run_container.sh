...
HOST_WEIGHTS_PATH="$(dirname "$0")/../weights"

echo "$HOST_WEIGHTS_PATH"

echo "正在啟動 Docker 容器: $CONTAINER_NAME"
docker run --name $CONTAINER_NAME --gpus all -p $PORT:8000 -v "$HOST_PATH":/app/src -v "$HOST_WEIGHTS_PATH":/app/weights $IMAGE_NAME 
